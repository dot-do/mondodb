...
description: Deploy mongo.do on Cloudflare Workers with Durable Objects for globally distributed document storage
---

# Cloudflare Workers Integration

mongo.do is designed from the ground up to run natively on Cloudflare Workers, leveraging Durable Objects with SQLite storage for globally distributed, MongoDB-compatible document storage. This guide covers everything you need to deploy and configure mongo.do on Cloudflare's edge network.

## Overview

mongo.do on Cloudflare Workers provides:
...
```typescript
import { MondoDatabase } from 'mongo.do/durable-object'
...
mongo.do uses a unified schema with two main tables:
...
Other Workers can communicate with mongo.do via service bindings:
...
mongo.do also exposes an HTTP RPC endpoint at `/rpc`:
...
mongo.do integrates with Cloudflare Vectorize for vector search capabilities.
...
```typescript
import { EmbeddingManager } from 'mongo.do/embedding'
...
mongo.do is compatible with Miniflare for local testing:
...
const mf = new Miniflare({
  modules: true,
  script: `
    import { MondoDatabase } from 'mongo.do'
    export { MondoDatabase }
    export default {
...
mongo.do exposes an MCP (Model Context Protocol) endpoint at `/mcp` on each Durable Object, allowing AI agents to interact with your database:
...
mongo.do uses an automatic migration system for SQLite schema management:
