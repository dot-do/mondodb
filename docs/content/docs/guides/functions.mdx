...
MondoDB supports the `$function` aggregation operator, allowing you to execute custom JavaScript functions within your aggregation pipelines. This enables complex transformations and computations that cannot be expressed using standard aggregation operators.
...
For pipelines processing many documents, MondoDB automatically optimizes function execution through batching. This reduces the overhead of creating isolated contexts for each document.
