...
MondoDB includes a MongoDB-compatible wire protocol server that enables connections from standard MongoDB tools and drivers. This allows you to use MongoDB Compass, mongosh, and any MongoDB driver with MondoDB for local development, debugging, and testing.
...
Use the MondoDB CLI to start a local wire protocol server:
...
npx mondodb serve --port 27018 --host 0.0.0.0

# With authentication
npx mondodb serve --auth --username admin --password secret

# With TLS
npx mondodb serve --tls --cert ./server.crt --key ./server.key
```
...
```typescript
import { WireProtocolServer, createServer } from 'mondodb/wire'
import { LocalSQLiteBackend } from 'mondodb/wire'

const backend = new LocalSQLiteBackend('.mondodb')
...
```typescript
import { createServer, LocalSQLiteBackend } from 'mondodb/wire'
...
```typescript
import { WireProtocolServer, LocalSQLiteBackend } from 'mondodb/wire'
...
MondoDB supports SCRAM-SHA-256 authentication, the same mechanism used by MongoDB.
...
MondoDB implements SCRAM-SHA-256 per RFC 5802 and RFC 7677:
...
```typescript
import { createScramCredentials, InMemoryCredentialsProvider } from 'mondodb/wire'
...
```typescript
import type { MondoBackend, DatabaseInfo, CollectionInfo } from 'mondodb/wire'
...
The MondoDB CLI provides convenient commands for the wire protocol server:
...
| `--data, -d` | `.mondodb` | Data directory for SQLite files |
