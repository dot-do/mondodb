---
title: Indexing
description: Create and manage indexes for optimal query performance in mongo.do
---

# Indexing

Indexes are essential for efficient query execution in mongo.do. This guide covers how to create, manage, and optimize indexes for your collections.

## Overview

mongo.do uses SQLite's indexing capabilities under the hood, with `json_extract` expressions to enable efficient querying on document fields. Every collection automatically has an index on the `_id` field, and you can create additional indexes to optimize your queries.

### How mongo.do Indexes Work with SQLite

When you create an index in mongo.do, it translates to a SQLite index on `json_extract` expressions:

```sql
-- A mongo.do index like { email: 1 }
...
mongo.do supports several index types:
...
mongo.do creates an FTS5 virtual table for text indexing:
...
mongo.do TTL indexes work as follows:
...
If you don't specify a name, mongo.do generates one based on the collection name, fields, and directions:
...
mongo.do's query optimizer selects the best index for each query. To help it:
...
mongo.do provides an `getIndexHints()` method to suggest which indexes could be used for a query:
...
mongo.do validates field paths to prevent SQL injection and ensure data integrity:
...
Since mongo.do stores indexes in SQLite:
