...
MondoDB provides a CLI command for starting an MCP server that communicates via stdio, making it easy to integrate with AI assistants like Claude Desktop.

### Using the MCP CLI Command

```bash
# Connect to a remote MondoDB Worker
...
| Option | Description |
|--------|-------------|
| `-c, --connection <url>` | Remote MondoDB Worker URL |
| `-l, --local <path>` | Local SQLite database path |

### Claude Desktop Configuration

Add MondoDB to your Claude Desktop configuration (`~/.config/claude/claude_desktop_config.json`):
...
```json
{
  "mcpServers": {
    "mondodb": {
      "command": "npx",
      "args": ["mondodb", "mcp", "--connection", "https://my-worker.workers.dev"]
    }
  }
}
```
...
```json
{
  "mcpServers": {
    "mondodb-local": {
      "command": "npx",
      "args": ["mondodb", "mcp", "--local", "./data/dev.db"]
    }
  }
}
```
...
```typescript
import { startMcpServer } from 'mondodb/cli/mcp'
...
When running on Cloudflare Workers, each MondoDB Durable Object exposes an MCP endpoint at `/mcp`. This allows AI agents to interact directly with the database:
...
- [Cloudflare Workers Integration](/docs/guides/cloudflare-workers) - Deploy MondoDB on Workers
