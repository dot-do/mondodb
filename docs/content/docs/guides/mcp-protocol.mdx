---
title: MCP Protocol
description: Integrate AI agents with mongo.do using the Model Context Protocol
---

# MCP Protocol

mongo.do provides native support for the Model Context Protocol (MCP), enabling AI agents to interact with your database through a standardized tool-calling interface. This guide covers how to create MCP servers, available database tools, and SDK adapters for popular AI frameworks.

## Overview

The Model Context Protocol (MCP) is a standard for AI agents to discover and invoke tools. mongo.do's MCP implementation provides:
...
```typescript
import { createMcpServer, type DatabaseAccess } from 'mongo.do/mcp'
...
| `name` | `string` | Server name (default: `'mongo.do'`) |
...
mongo.do's MCP server provides three built-in tools for AI agents:
...
mongo.do provides adapters for integrating MCP servers with popular AI SDKs.
...
import { createMcpServer, createVercelAdapter } from 'mongo.do/mcp'
...
```typescript
import { createMcpServer, createWorkerEvaluator } from 'mongo.do/mcp'
...
```typescript
import { createMcpServer, createHttpMcpHandler } from 'mongo.do/mcp'
...
```typescript
import { createMcpServer, createStdioTransport } from 'mongo.do/mcp'
...
```typescript
import { McpError, McpErrorCode, isRetryableError } from 'mongo.do/mcp'
...
mongo.do provides a CLI command for starting an MCP server that communicates via stdio, making it easy to integrate with AI assistants like Claude Desktop.

### Using the MCP CLI Command

```bash
# Connect to a remote mongo.do Worker
mongo.do mcp --connection https://my-worker.workers.dev

# Use a local SQLite database (requires Bun runtime)
...
| Option | Description |
|--------|-------------|
| `-c, --connection <url>` | Remote mongo.do Worker URL |
| `-l, --local <path>` | Local SQLite database path |

### Claude Desktop Configuration

Add mongo.do to your Claude Desktop configuration (`~/.config/claude/claude_desktop_config.json`):
...
```json
{
  "mcpServers": {
    "mongo.do": {
      "command": "npx",
      "args": ["mongo.do", "mcp", "--connection", "https://my-worker.workers.dev"]
    }
  }
}
```
...
```json
{
  "mcpServers": {
    "mongo.do-local": {
      "command": "npx",
      "args": ["mongo.do", "mcp", "--local", "./data/dev.db"]
    }
  }
}
```
...
```typescript
import { startMcpServer } from 'mongo.do/cli/mcp'
...
## Durable Object MCP Endpoint

When running on Cloudflare Workers, each mongo.do Durable Object exposes an MCP endpoint at `/mcp`. This allows AI agents to interact directly with the database:

```typescript
// The MCP endpoint is automatically available
// POST https://your-worker.workers.dev/mcp
```

The Durable Object MCP handler is created lazily on first request:

```typescript
// Inside MondoDatabase.fetch()
...
- [Cloudflare Workers Integration](/docs/guides/cloudflare-workers) - Deploy mongo.do on Workers
- [CLI Guide](/docs/guides/cli) - Command-line interface reference
- [RPC Protocol](/docs/guides/rpc) - RPC communication details
