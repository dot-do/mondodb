...
description: Implement powerful full-text search in MondoDB using SQLite FTS5
---
...
MondoDB supports several search operators for different use cases.
...
MondoDB uses the BM25 algorithm for relevance scoring. Results are automatically sorted by relevance score (highest first).
...
MondoDB supports highlighting matched terms and extracting relevant snippets from search results.
...
Combine full-text search with vector similarity search for improved results. MondoDB supports two fusion algorithms:
...
Under the hood, MondoDB translates search operators to SQLite FTS5 MATCH syntax:
