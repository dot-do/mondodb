...
description: Connect to MondoDB and manage database connections
---

# MongoClient & Connection

The `MongoClient` class is the main entry point for connecting to MondoDB. It provides a MongoDB-compatible API that works in two distinct modes:
...
MondoDB supports both `mondodb://` and `mongodb://` URI schemes:
...
|-----------|-------------|---------|
| `scheme` | Protocol (`mondodb://` or `mongodb://`) | Required |
...
```typescript
// Minimal connection
const client = new MongoClient('mondodb://localhost')
...
// With authentication
const client = new MongoClient('mondodb://admin:secret@localhost:27017/myapp')

// With options
const client = new MongoClient('mondodb://localhost:27017/myapp?maxPoolSize=50&minPoolSize=5')
```
...
Establishes the connection to the database. In MondoDB, connections are handled lazily, so this primarily initializes the client state.
...
```typescript
const client = new MongoClient('mondodb://localhost:27017/defaultdb')
await client.connect()
...
MondoDB supports MongoDB-compatible sessions for transaction management:
