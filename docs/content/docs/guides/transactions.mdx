...
mongo.do supports multi-document transactions, providing ACID guarantees for operations that need to be atomic across multiple documents or collections. Transactions ensure that either all operations succeed together, or none of them take effect.

## Overview

Transactions in mongo.do work through sessions. A session tracks the state of a transaction and ensures that all operations within the transaction are executed atomically.

### How Transactions Work in mongo.do

mongo.do maps MongoDB transaction semantics to SQLite transactions:
...
Understanding transaction scope is important when using mongo.do with Cloudflare Durable Objects:
...
In Durable Objects mode, mongo.do uses SQLite's `transactionSync()`:
...
### Current Limitations in mongo.do
