...
mongo.do integrates with Cloudflare Vectorize and Workers AI to provide powerful vector search capabilities. This enables semantic similarity search, document recommendations, and retrieval-augmented generation (RAG) applications.
...
### mongo.do Configuration

Configure mongo.do to use Vectorize for automatic embedding generation:
...
mongo.do uses Cloudflare Workers AI to generate embeddings. The default model is `@cf/baai/bge-m3`, which offers:
...
mongo.do can automatically generate and store embeddings when documents are inserted or updated.
...
Before generating embeddings, documents are serialized into text. mongo.do provides flexible serialization options.
