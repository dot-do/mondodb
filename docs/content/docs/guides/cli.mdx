---
title: CLI Server
description: Run mongo.do locally with the CLI server for development and testing
---

# CLI Server

The mongo.do CLI provides a command-line interface for running a MongoDB-compatible wire protocol server locally. It supports two modes of operation: local SQLite storage for development, and remote proxy mode for connecting to Cloudflare Workers deployments.
...
This makes mongo.do ideal for local development while maintaining compatibility with your production Cloudflare Workers deployment.

## Installation

Install mongo.do globally or as a project dependency:

```bash
# Global installation
npm install -g mongo.do
...
mongo.do serve --data /var/lib/mongo.do
...
mongo.do serve --remote https://my-mongo.do.workers.dev/rpc

# Proxy with local port customization
mongo.do serve --remote https://my-mongo.do.workers.dev/rpc --port 27018 --verbose
```

### RPC Endpoint

Your Cloudflare Workers deployment must expose an RPC endpoint. mongo.do automatically handles:
...
mongo.do serve --remote https://staging.my-mongo.do.workers.dev/rpc

# Production debugging: Proxy with verbose logging
mongo.do serve --remote https://my-mongo.do.workers.dev/rpc --verbose
```
...
# With custom port
mongo.do serve --host 0.0.0.0 --port 27017
```
...
mongo.do serve --port 27018 --data ./test-data
...
1. Start the CLI server: `mongo.do serve`
...
jobs:
  test:
    ...
    steps:
      ...
      - name: Start mongo.do
        run: bun run mongo.do serve --port 27017 &
...
```typescript
import { parseArgs, runServer, validateOptions } from 'mongo.do/cli/server'
...
For production workloads, deploy mongo.do to Cloudflare Workers with Durable Objects.
