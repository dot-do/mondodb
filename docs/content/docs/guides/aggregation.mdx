...
description: Comprehensive guide to MongoDB-style aggregation pipelines in mongo.do
---

# Aggregation Pipeline

mongo.do supports MongoDB's aggregation pipeline framework, allowing you to process and transform documents through a series of stages. The aggregation framework translates MongoDB pipeline syntax to optimized SQL queries executed against SQLite, providing familiar MongoDB semantics with the reliability of SQL.
...
mongo.do supports the following aggregation pipeline stages:
...
While mongo.do supports most common aggregation operations, there are some differences from MongoDB:

| Feature | mongo.do Support | Notes |
|---------|-----------------|-------|
