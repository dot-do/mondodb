...
TTL (Time-To-Live) indexes enable automatic document expiration in mongo.do. Documents are automatically deleted after a specified number of seconds, making TTL indexes ideal for session management, cache entries, temporary data, and log rotation.

## Overview

A TTL index is a special single-field index that mongo.do uses to automatically remove documents from a collection after a specified amount of time. The index monitors a date field in each document and removes documents once the time has elapsed.
...
mongo.do calculates the expiration time for each document as:
...
The TTL field must contain a valid date value. mongo.do supports:
...
mongo.do runs a background process that:
