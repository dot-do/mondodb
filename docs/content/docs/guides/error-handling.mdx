...
description: Handle errors gracefully and implement retry logic in mongo.do
---

# Error Handling

mongo.do provides MongoDB-compatible error handling with specialized error classes, error codes, and built-in retry mechanisms. This guide covers error types, handling patterns, and strategies for building robust applications.

## Error Types

mongo.do includes several error classes for different failure scenarios:
...
```typescript
import { BulkWriteException, BulkWriteError } from 'mongo.do/client'
...
```typescript
import { AggregationError } from 'mongo.do/client'
...
```typescript
import { ImmutableEntryError } from 'mongo.do/agentfs'
...
For MCP (Model Context Protocol) integrations, mongo.do provides specialized error classes:
...
import {
  ...
  ToolExecutionError,
  InvalidParamsError,
} from 'mongo.do/mcp/adapters/errors'
...
mongo.do uses MongoDB-compatible error codes:

```typescript
import { ErrorCode, getErrorCodeName } from 'mongo.do/types/rpc'
...
```typescript
import { McpErrorCode } from 'mongo.do/mcp/adapters/errors'
...
Transactions can fail due to conflicts with concurrent operations. mongo.do supports automatic retry for transient transaction errors:
...
```typescript
import { isRetryableError, getRetryDelay } from 'mongo.do/mcp/adapters/errors'
...
```typescript
import { classifyError, McpErrorCode } from 'mongo.do/mcp/adapters/errors'
...
3. **Use withTransaction** - Let mongo.do handle transaction retries automatically
