...
description: Complete guide to mongo.do query operators and filter syntax
---

# Query & Filter Syntax

mongo.do supports MongoDB-compatible query syntax for filtering documents. This guide covers all supported query operators with examples.
...
mongo.do automatically optimizes queries:
...
mongo.do provides full TypeScript support for type-safe queries.

### Typed Filters

```typescript
import type { Filter } from 'mongo.do'
...
```typescript
import type { FilterOperators } from 'mongo.do'
...
```typescript
import type { RootFilterOperators } from 'mongo.do'
