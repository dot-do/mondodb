...
description: Run analytical queries on your mongo.do data using ClickHouse with Apache Iceberg
---

# Analytics Layer

mongo.do includes an integrated analytics layer that streams your operational data to ClickHouse via Change Data Capture (CDC), enabling real-time analytical queries on Apache Iceberg tables stored in R2.
...
The `$analytics` stage is transparent — you write MongoDB-style pipelines, and mongo.do translates them to optimized ClickHouse SQL:
...
1. **Capture** — Every insert, update, and delete in mongo.do generates a CDC event
...
mongo.do stores analytical data in Apache Iceberg format on R2.
...
```typescript
import { createIcebergConnection } from 'mongo.do/olap'
...
```typescript
import { R2DataCatalog } from 'mongo.do/olap'
...
```typescript
import { ClickHouseQueryExecutor } from 'mongo.do/olap'
...
```typescript
import { createQueryBuilder } from 'mongo.do/olap'
...
```jsonc
{
  ...
  "r2_buckets": [
    { "binding": "R2_BUCKET", "bucket_name": "mongo.do-iceberg" }
  ],

  // Pipeline for CDC
  "pipelines": [
    { "binding": "CDC_PIPELINE", "pipeline": "mongo.do-cdc" }
  ],
...
ENGINE = Iceberg('https://your-account.r2.cloudflarestorage.com/mongo.do-iceberg', 'access_key', 'secret_key')
SETTINGS
  catalog_type = 'rest',
...
CREATE TABLE cdc_queue (
  ...
  'https://your-account.r2.cloudflarestorage.com/mongo.do-cdc/*',
