...
```typescript
// Basic connection
new MongoClient('mondodb://localhost/mydb')
// host: localhost, port: 27017, database: mydb

// With port
new MongoClient('mondodb://localhost:27018/mydb')

// With authentication
new MongoClient('mondodb://user:pass@localhost/mydb')

// With options
new MongoClient('mondodb://localhost/mydb?maxPoolSize=50')
```
...
When using Durable Objects mode, the environment must include the `MONDO_DATABASE` binding:

```typescript
interface Env {
  MONDO_DATABASE: DurableObjectNamespace
  LOADER?: WorkerLoader  // Optional: for $function operator
}
```
...
**Note**: In Cloudflare Workers, connection state is per-request. The `close()` method clears internal caches but doesn't affect Durable Object connections.
