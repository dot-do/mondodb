...
// With authentication
const client = new MongoClient('mondodb://user:pass@localhost:27017/mydb')

// With options in query string
const client = new MongoClient('mondodb://localhost:27017/mydb?maxPoolSize=50')
...
**URI Format:**
```
mondodb://[username:password@]host[:port][/database][?options]
```

| Component | Default | Description |
|-----------|---------|-------------|
| `scheme` | - | `mondodb://` or `mongodb://` |
...
const client = new MongoClient(
  'mondodb://localhost:27017/mydb?maxPoolSize=50',
  { maxPoolSize: 100 }
)
```
...
```typescript
// URI mode
const client1 = new MongoClient('mondodb://localhost:27017')
...
```typescript
const client = new MongoClient('mondodb://localhost:27017/mydb')
console.log(client.uri) // 'mondodb://localhost:27017/mydb'
...
const client2 = new MongoClient('mondodb://localhost:27017')
...
**Behavior:**
- In MondoDB, connections are handled lazily, so this is mostly a no-op
...
MondoDB parses connection URIs according to MongoDB connection string format.

### Supported URI Components

```typescript
interface ParsedURI {
  scheme: string              // 'mondodb' or 'mongodb'
...
// Minimal
'mondodb://localhost'
// -> { scheme: 'mondodb', host: 'localhost', port: 27017, database: 'test' }

// With port and database
'mondodb://localhost:27018/mydb'
// -> { scheme: 'mondodb', host: 'localhost', port: 27018, database: 'mydb' }

// With authentication
'mondodb://admin:secret123@localhost:27017/mydb'
// -> { username: 'admin', password: 'secret123', ... }

// With URL-encoded password (special characters)
'mondodb://admin:p%40ssw%2Frd@localhost:27017/mydb'
// -> { password: 'p@ssw/rd', ... }

// With options
'mondodb://localhost:27017/mydb?maxPoolSize=50&minPoolSize=5'
...
// Error: Invalid URI scheme: Expected "mondodb://" or "mongodb://", got "postgres://"
...
// Invalid port
new MongoClient('mondodb://localhost:abc')
// Error: Invalid port: "abc"
```

## Wire Protocol Support

MondoDB includes wire protocol support for compatibility with MongoDB tools and drivers.

### Connecting with Wire Protocol

When running the MondoDB server locally, standard MongoDB clients can connect:
...
To enable wire protocol, run the MondoDB CLI server:

```bash
# Start the MondoDB server
npx mondodb serve --port 27017 --data-dir .mondodb
```
...
```typescript
try {
  const client = new MongoClient('mondodb://invalid-host:27017')
  await client.connect()
} catch (error) {
