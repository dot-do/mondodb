...
Detailed documentation for all MondoDB APIs and methods. MondoDB provides a MongoDB-compatible API that runs on Cloudflare Workers with SQLite-backed Durable Objects.

## Quick Start

```typescript
import { MongoClient, ObjectId } from 'mondodb'
...
MondoDB aims for high compatibility with the MongoDB Node.js driver API. Key supported features:
...
MondoDB is written in TypeScript and provides full type definitions:
