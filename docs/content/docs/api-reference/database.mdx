...
The `Database` and `MongoDatabase` classes represent a mongo.do database and provide methods for working with collections. `Database` is used with Cloudflare Durable Objects (production), while `MongoDatabase` is used with URI connections (testing/in-memory).

## Import

```typescript
import { Database, MongoDatabase } from 'mongo.do'
```
...
### URI Mode (`new MongoClient('mongodo://...')`)
